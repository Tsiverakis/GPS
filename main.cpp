...
#include "modules/world/gridworld.h"
#include "modules/car/car.h"
#include "modules/objects/StaticObjects.h"
#include "modules/objects/MovingObjects.h"
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        if (arg == "--help")
        {
            ...
            minConfidenceThreshold = stod(argv[++i]) / 100.0;
        else if (arg == "--gps")
        {
            int j = i + 1;
    ...
    // Providing: Position, Target, Type, ID, and Glyph
    SelfDrivingCar *egoCar = new SelfDrivingCar(Position(0, 0), gpsTargets[0], "MovingObject", "SelfDrivingCar:1", "@");
...
\
    ...
    for (int tick = 0; tick < simulationTicks; ++tick)
    {
        ...
        world.update(count_for_trafficlight_ticks);
        ...
        if (egoCar->getPosition().getX() == gpsTargets[0].getX() && egoCar->getPosition().getY() == gpsTargets[0].getY())
        {
            break;
        }
